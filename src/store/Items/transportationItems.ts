import {TTransportationItemsInfo} from "../../types/transportationTypes";

export const transportationItems: TTransportationItemsInfo = {
    "MAIN": [
        {
            itemId: 'FROSTSTAFF',
            itemName: {
                ru: 'Морозный посох',
                en: 'Frost Staff',
            },
        },
        {
            itemId: 'FROSTSTAFF_KEEPER',
            itemName: {
                ru: 'Посох инея',
                en: 'Hoarfrost Staff',
            },
        },
        {
            itemId: 'FROSTSTAFF_AVALON',
            itemName: {
                ru: 'Леденящий вой',
                en: 'Chillhowl',
            },
        },
        {
            itemId: 'ARCANESTAFF',
            itemName: {
                ru: 'Мистический посох',
                en: 'Arcane Staff',
            },
        },
        {
            itemId: 'ARCANESTAFF_UNDEAD',
            itemName: {
                ru: 'Ведьмовской посох',
                en: 'Witchwork Staff',
            },
        },
        {
            itemId: 'CURSEDSTAFF',
            itemName: {
                ru: 'Проклятый посох',
                en: 'Cursed Staff',
            },
        },
        {
            itemId: 'CURSEDSTAFF_UNDEAD',
            itemName: {
                ru: 'Гибельный посох',
                en: 'Lifecurse Staff',
            },
        },
        {
            itemId: 'CURSEDSTAFF_AVALON',
            itemName: {
                ru: 'Призыватель теней',
                en: 'Shadowcaller',
            },
        },
        {
            itemId: 'FIRESTAFF',
            itemName: {
                ru: 'Огненный посох',
                en: 'Fire Staff',
            },
        },
        {
            itemId: 'FIRESTAFF_KEEPER',
            itemName: {
                ru: 'Посох лесного пожара',
                en: 'Wildfire Staff',
            },
        },
        {
            itemId: 'HOLYSTAFF',
            itemName: {
                ru: 'Священный посох',
                en: 'Holy Staff',
            },
        },
        {
            itemId: 'HOLYSTAFF_MORGANA',
            itemName: {
                ru: 'Посох жизни',
                en: 'Lifetouch Staff',
            },
        },
        {
            itemId: 'HOLYSTAFF_AVALON',
            itemName: {
                ru: 'Благодатный посох',
                en: 'Hallowfall',
            },
        },
        {
            itemId: 'NATURESTAFF',
            itemName: {
                ru: 'Древесный посох',
                en: 'Nature Staff',
            },
        },
        {
            itemId: 'NATURESTAFF_KEEPER',
            itemName: {
                ru: 'Друидский посох',
                en: 'Druidic Staff',
            },
        },
        {
            itemId: 'NATURESTAFF_AVALON',
            itemName: {
                ru: 'Посох железного корня',
                en: 'Ironroot Staff',
            },
        },
        {
            itemId: 'AXE',
            itemName: {
                ru: 'Боевой топор',
                en: 'Battleaxe',
            },
        },
        {
            itemId: 'DAGGER',
            itemName: {
                ru: 'Кинжал',
                en: 'Dagger',
            },
        },
        {
            itemId: 'RAPIER_MORGANA',
            itemName: {
                ru: 'Кровопускатель',
                en: 'Bloodletter',
            },
        },
        {
            itemId: 'DAGGER_HELL',
            itemName: {
                ru: 'Клык демона',
                en: 'Demonfang',
            },
        },
        {
            itemId: 'HAMMER',
            itemName: {
                ru: 'Молот',
                en: 'Hammer',
            },
        },
        {
            itemId: 'MACE',
            itemName: {
                ru: 'Булава',
                en: 'Mace',
            },
        },
        {
            itemId: 'ROCKMACE_KEEPER',
            itemName: {
                ru: 'Каменная булава',
                en: 'Bedrock Mace',
            },
        },
        {
            itemId: 'MACE_HELL',
            itemName: {
                ru: 'Булава инкуба',
                en: 'Incubus Mace',
            },
        },
        {
            itemId: 'SPEAR',
            itemName: {
                ru: 'Копье',
                en: 'Spear',
            },
        },
        {
            itemId: 'SPEAR_KEEPER',
            itemName: {
                ru: 'Копье цапли',
                en: 'Heron Spear',
            },
        },
        {
            itemId: 'SPEAR_LANCE_AVALON',
            itemName: {
                ru: 'Копье рассвета',
                en: 'Daybreaker',
            },
        },
        {
            itemId: 'SWORD',
            itemName: {
                ru: 'Палаш',
                en: 'Broadsword',
            },
        },
        {
            itemId: 'SCIMITAR_MORGANA',
            itemName: {
                ru: 'Королевский меч',
                en: 'Clarent Blade',
            },
        },
        {
            itemId: '1HCROSSBOW',
            itemName: {
                ru: 'Легкий арбалет',
                en: 'Light Crossbow',
            },
        },
    ],
    "2H": [
        {
            itemId: 'FROSTSTAFF',
            itemName: {
                ru: 'Большой морозный посох',
                en: 'Great Frost Staff',
            },
        },
        {
            itemId: 'GLACIALSTAFF',
            itemName: {
                ru: 'Ледяной посох',
                en: 'Glacial Staff',
            },

        },
        {
            itemId: 'ICEGAUNTLETS_HELL',
            itemName: {
                ru: 'Заледенелый посох',

                en: 'Icicle Staff',

            },
        },
        {
            itemId: 'ICECRYSTAL_UNDEAD',
            itemName: {
                ru: 'Призма вечного холода',
                en: 'Permafrost Prism',

            },
        },
        {
            itemId: 'ARCANESTAFF',
            itemName: {
                ru: 'Большой мистический посох',
                en: 'Great Arcane Staff',

            },
        },
        {
            itemId: 'ENIGMATICSTAFF',
            itemName: {
                ru: 'Загадочный посох',
                en: 'Enigmatic Staff',

            },
        },
        {
            itemId: 'ARCANESTAFF_HELL',
            itemName: {
                ru: 'Оккультный посох',
                en: 'Occult Staff',

            },
        },
        {
            itemId: 'ENIGMATICORB_MORGANA',
            itemName: {
                ru: 'Средоточие злобы',
                en: 'Malevolent Locus',

            },
        },
        {
            itemId: 'ARCANE_RINGPAIR_AVALON',
            itemName: {
                ru: 'Песнь заката',
                en: 'Evensong',

            },
        },
        {
            itemId: 'CURSEDSTAFF',
            itemName: {
                ru: 'Большой проклятый посох',
                en: 'Great Cursed Staff',

            },
        },
        {
            itemId: 'DEMONICSTAFF',
            itemName: {
                ru: 'Демонический посох',
                en: 'Demonic Staff',

            },
        },
        {
            itemId: 'SKULLORB_HELL',
            itemName: {
                ru: 'Проклятый череп',
                en: 'Cursed Skull',

            },
        },
        {
            itemId: 'CURSEDSTAFF_MORGANA',
            itemName: {
                ru: 'Посох обреченности',
                en: 'Damnation Staff',

            },
        },
        {
            itemId: 'FIRESTAFF',
            itemName: {
                ru: 'Большой огненный посох (Гнев Vendetta)',
                en: 'Great Fire Staff(Vendetta\'s Wrath)',

            },
        },
        {
            itemId: 'INFERNOSTAFF',
            itemName: {
                ru: 'Адский посох',
                en: 'Infernal Staff',

            },
        },
        {
            itemId: 'FIRESTAFF_HELL',
            itemName: {
                ru: 'Посох серы',
                en: 'Brimstone Staff',

            },
        },
        {
            itemId: 'INFERNOSTAFF_MORGANA',
            itemName: {
                ru: 'Пылающий посох',
                en: 'Blazing Staff',

            },
        },
        {
            itemId: 'FIRE_RINGPAIR_AVALON',
            itemName: {
                ru: 'Песнь рассвета',
                en: 'Dawnsong',

            },
        },
        {
            itemId: 'HOLYSTAFF',
            itemName: {
                ru: 'Большой священный посох',
                en: 'Great Holy Staff',

            },
        },
        {
            itemId: 'DIVINESTAFF',
            itemName: {
                ru: 'Божественный посох',
                en: 'Divine Staff',

            },
        },
        {
            itemId: 'HOLYSTAFF_HELL',
            itemName: {
                ru: 'Посох падших',
                en: 'Fallen Staff',

            },
        },
        {
            itemId: 'HOLYSTAFF_UNDEAD',
            itemName: {
                ru: 'Посох искупления',
                en: 'Redemption Staff',

            },
        },
        {
            itemId: 'NATURESTAFF',
            itemName: {
                ru: 'Большой древесный посох',
                en: 'Great Nature Staff',

            },
        },
        {
            itemId: 'WILDSTAFF',
            itemName: {
                ru: 'Дикий посох',
                en: 'Wild Staff',

            },
        },
        {
            itemId: 'NATURESTAFF_HELL',
            itemName: {
                ru: 'Посох порчи',
                en: 'Blight Staff',

            },
        },
        {
            itemId: 'NATURESTAFF_KEEPER',
            itemName: {
                ru: 'Неудержимый посох',
                en: 'Rampant Staff',

            },
        },
        {
            itemId: 'AXE',
            itemName: {
                ru: 'Большой топор(Рука Кхора)',
                en: 'Greataxe/The Hand of Khor',

            },
        },
        {
            itemId: 'HALBERD',
            itemName: {
                ru: 'Алебарда',
                en: 'Halberd',

            },
        },
        {
            itemId: 'HALBERD_MORGANA',
            itemName: {
                ru: 'Падальщик',
                en: 'Carrioncaller',

            },
        },
        {
            itemId: 'SCYTHE_HELL',
            itemName: {
                ru: 'Адская коса',
                en: 'Infernal Scythe',

            },
        },
        {
            itemId: 'DUALAXE_KEEPER',
            itemName: {
                ru: 'Медвежьи лапы',
                en: 'Bear Paws',

            },
        },
        {
            itemId: 'AXE_AVALON',
            itemName: {
                ru: 'Разрушитель миров',
                en: 'Realmbreaker',

            },
        },
        {
            itemId: 'DAGGERPAIR',
            itemName: {
                ru: 'Парные кинжалы',
                en: 'Dagger Pair',

            },
        },
        {
            itemId: 'CLAWPAIR',
            itemName: {
                ru: 'Когти',
                en: 'Claws',

            },
        },
        {
            itemId: 'DUALSICKLE_UNDEAD',
            itemName: {
                ru: 'Вестники смерти',
                en: 'Deathgivers',

            },
        },
        {
            itemId: 'DAGGER_KATAR_AVALON',
            itemName: {
                ru: 'Укротитель ярости',
                en: 'Bridled Fury',

            },
        },
        {
            itemId: 'POLEHAMMER',
            itemName: {
                ru: 'Чекан',
                en: 'Polehammer',

            },
        },
        {
            itemId: 'HAMMER',
            itemName: {
                ru: 'Большой молот',
                en: 'Great Hammer',

            },
        },
        {
            itemId: 'HAMMER_UNDEAD',
            itemName: {
                ru: 'Могильный молот',
                en: 'Tombhammer',

            },
        },
        {
            itemId: 'DUALHAMMER_HELL',
            itemName: {
                ru: 'Молоты кузни',
                en: 'Forge Hammers',

            },
        },
        {
            itemId: 'HAMMER_AVALON',
            itemName: {
                ru: 'Длань правосудия',
                en: 'Hand of Justice',

            },
        },
        {
            itemId: 'RAM_KEEPER',
            itemName: {
                ru: 'Хранитель рощи',
                en: 'Grovekeeper',

            },
        },
        {
            itemId: 'KNUCKLES_SET1',
            itemName: {
                ru: 'Перчатки крушителя',
                en: 'Brawler Gloves',

            },
        },
        {
            itemId: 'KNUCKLES_SET2',
            itemName: {
                ru: 'Боевые наручи',
                en: 'Battle Bracers',

            },
        },
        {
            itemId: 'KNUCKLES_SET3',
            itemName: {
                ru: 'Шипастые рукавицы',
                en: 'Spiked Gauntlets',

            },
        },
        {
            itemId: 'KNUCKLES_KEEPER',
            itemName: {
                ru: 'Медвежьи перчатки',
                en: 'Ursine Maulers',

            },
        },
        {
            itemId: 'KNUCKLES_HELL',
            itemName: {
                ru: 'Адские длани',
                en: 'Hellfire Hands',

            },
        },
        {
            itemId: 'KNUCKLES_MORGANA',
            itemName: {
                ru: 'Вороний цестус',
                en: 'Ravenstrike Cestus',

            },
        },
        {
            itemId: 'KNUCKLES_AVALON',
            itemName: {
                ru: 'Кулаки Авалона',
                en: 'Fists of Avalon',

            },
        },
        {
            itemId: 'MACE',
            itemName: {
                ru: 'Тяжелая булава',
                en: 'Heavy Mace',

            },
        },
        {
            itemId: 'FLAIL',
            itemName: {
                ru: 'Моргенштерн',
                en: 'Morning Star',

            },
        },
        {
            itemId: 'MACE_MORGANA',
            itemName: {
                ru: 'Булава Камланна',
                en: 'Camlann Mace',

            },
        },
        {
            itemId: 'DUALMACE_AVALON',
            itemName: {
                ru: 'Хранители клятвы',
                en: 'Oathkeepers',

            },
        },
        {
            itemId: 'QUARTERSTAFF',
            itemName: {
                ru: 'Боевой шест',
                en: 'Quarterstaff',

            },
        },
        {
            itemId: 'IRONCLADEDSTAFF',
            itemName: {
                ru: 'Железный посох',
                en: 'Iron-clad Staff',

            },
        },
        {
            itemId: 'DOUBLEBLADEDSTAFF',
            itemName: {
                ru: 'Острый шест',
                en: 'Double Bladed Staff',

            },
        },
        {
            itemId: 'COMBATSTAFF_MORGANA',
            itemName: {
                ru: 'Посох черного монаха',
                en: 'Black Monk Stave',

            },
        },
        {
            itemId: 'TWINSCYTHE_HELL',
            itemName: {
                ru: 'Коса душ',
                en: 'Soulscythe',

            },
        },
        {
            itemId: 'ROCKSTAFF_KEEPER',
            itemName: {
                ru: 'Посох равновесия',
                en: 'Staff of Balance',

            },
        },
        {
            itemId: 'QUARTERSTAFF_AVALON',
            itemName: {
                ru: 'Искатель Грааля',
                en: 'Grailseeker',

            },
        },
        {
            itemId: 'SPEAR',
            itemName: {
                ru: 'Пика',
                en: 'Pike',

            },
        },
        {
            itemId: 'GLAIVE',
            itemName: {
                ru: 'Глефа',
                en: 'Glaive',

            },
        },
        {
            itemId: 'HARPOON_HELL',
            itemName: {
                ru: 'Охотник за душами',
                en: 'Spirithunter',

            },
        },
        {
            itemId: 'TRIDENT_UNDEAD',
            itemName: {
                ru: 'Трезубец',
                en: 'Trinity Spear',

            },
        },
        {
            itemId: 'CLAYMORE',
            itemName: {
                ru: 'Клеймор',
                en: 'Claymore',

            },
        },
        {
            itemId: 'DUALSWORD',
            itemName: {
                ru: 'Парные мечи',
                en: 'Dual Swords',

            },
        },
        {
            itemId: 'CLEAVER_HELL',
            itemName: {
                ru: 'Меч резни',
                en: 'Carving Sword',

            },
        },
        {
            itemId: 'DUALSCIMITAR_UNDEAD',
            itemName: {
                ru: 'Парные галатины',
                en: 'Galatine Pair',

            },
        },
        {
            itemId: 'CLAYMORE_AVALON',
            itemName: {
                ru: 'Царетворец',
                en: 'Kingmaker',

            },
        },
        {
            itemId: 'BOW',
            itemName: {
                ru: 'Лук',
                en: 'Bow',

            },
        },
        {
            itemId: 'WARBOW',
            itemName: {
                ru: 'Боевой лук',
                en: 'Warbow',

            },
        },
        {
            itemId: 'LONGBOW',
            itemName: {
                ru: 'Длинный лук',
                en: 'Longbow',

            },
        },
        {
            itemId: 'LONGBOW_UNDEAD',
            itemName: {
                ru: 'Шепчущий лук',
                en: 'Whispering Bow',

            },
        },
        {
            itemId: 'BOW_HELL',
            itemName: {
                ru: 'Воющий лук',
                en: 'Wailing Bow',

            },
        },
        {
            itemId: 'BOW_KEEPER',
            itemName: {
                ru: 'Лук Бадона',
                en: 'Bow of Badon',

            },
        },
        {
            itemId: 'BOW_AVALON',
            itemName: {
                ru: 'Туманный пронзатель',
                en: 'Mistpiercer',

            },
        },
        {
            itemId: 'CROSSBOW',
            itemName: {
                ru: 'Арбалет',
                en: 'Crossbow',

            },
        },
        {
            itemId: 'CROSSBOWLARGE',
            itemName: {
                ru: 'Тяжелый арбалет',
                en: 'Heavy Crossbow',

            },
        },
        {
            itemId: 'REPEATINGCROSSBOW_UNDEAD',
            itemName: {
                ru: 'Плачущий арбалет',
                en: 'Weeping Repeater',

            },
        },
        {
            itemId: 'DUALCROSSBOW_HELL',
            itemName: {
                ru: 'Самострелы',
                en: 'Boltcasters',

            },
        },
        {
            itemId: 'CROSSBOWLARGE_MORGANA',
            itemName: {
                ru: 'Осадный арбалет',
                en: 'Siegebow',

            },
        },
        {
            itemId: 'CROSSBOW_CANNON_AVALON',
            itemName: {
                ru: 'Творец энергии',
                en: 'Energy Shaper',

            },
        },
        {
            itemId: 'TOOL_FISHINGROD',
            itemName: {
                ru: 'Удочка',
                en: 'Fishing Rod',

            },
        },
        {
            itemId: 'TOOL_FISHINGROD_AVALON',
            itemName: {
                ru: 'Авалонская удочка',
                en: 'Avalonian Fishing Rod',

            },
        },
        {
            itemId: 'TOOL_SIEGEHAMMER',
            itemName: {
                ru: 'Осадный молот',
                en: 'Demolition Hammer',

            },
        },
        {
            itemId: 'SIEGEHAMMER_AVALON',
            itemName: {
                ru: 'Авалонский осадный молот',
                en: 'Avalonian Demolition Hammer',

            },
        },
        {
            itemId: 'TOOL_PICK',
            itemName: {
                ru: 'Кирка',
                en: 'Pickaxe',

            },
        },
        {
            itemId: 'TOOL_PICK_AVALON',
            itemName: {
                ru: 'Авалонская кирка',
                en: 'Avalonian Pickaxe',

            },
        },
        {
            itemId: 'TOOL_SICKLE',
            itemName: {
                ru: 'Серп',
                en: 'Sickle',

            },
        },
        {
            itemId: 'TOOL_SICKLE_AVALON',
            itemName: {
                ru: 'Авалонский серп',
                en: 'Avalonian Sickle',

            },
        },
        {
            itemId: 'TOOL_KNIFE',
            itemName: {
                ru: 'Свежевальный нож',
                en: 'Skinning Knife',

            },
        },
        {
            itemId: 'TOOL_KNIFE_AVALON',
            itemName: {
                ru: 'Авалонский свежевальный нож',
                en: 'Avalonian Skinning Knife',

            },
        },
        {
            itemId: 'TOOL_HAMMER',
            itemName: {
                ru: 'Кувалда',
                en: 'Stone Hammer',

            },
        },
        {
            itemId: 'TOOL_HAMMER_AVALON',
            itemName: {
                ru: 'Авалонская кувалда',
                en: 'Avalonian Stone Hammer',

            },
        },
        {
            itemId: 'TOOL_AXE',
            itemName: {
                ru: 'Топор',
                en: 'Axe',

            },
        },
        {
            itemId: 'TOOL_AXE_AVALON',
            itemName: {
                ru: 'Авалонский топор',
                en: 'Avalonian Axe',

            },
        },
    ],
    "BAG": [
        {
            itemId: 'BAG',
            itemName: {
                ru: 'Сумка',
                en: 'Bag',

            },
        },
        {
            itemId: 'INSIGHT',
            itemName: {
                ru: 'Кошель интуиции',
                en: 'Satchel of Insight',

            },
        },
    ],
    "CAPE": [
        {
            itemId: 'CAPE',
            itemName: {
                ru: 'Плащ',
                en: 'Cape',

            },
        },
    ],
    "CAPEITEM": [
        {
            itemId: 'FW_BRIDGEWATCH',
            itemName: {
                ru: 'Накидка города Bridgewatch',
                en: 'Bridgewatch Cape',

            },
        },
        {
            itemId: 'FW_FORTSTERLING',
            itemName: {
                ru: 'Накидка города Fort Sterling',
                en: 'Fort Sterling Cape',

            },
        },
        {
            itemId: 'FW_LYMHURST',
            itemName: {
                ru: 'Накидка города Lymhurst',
                en: 'Lymhurst Cape',

            },
        },
        {
            itemId: 'FW_MARTLOCK',
            itemName: {
                ru: 'Накидка города Martlock',
                en: 'Martlock Cape',

            },
        },
        {
            itemId: 'FW_THETFORD',
            itemName: {
                ru: 'Накидка города Thetford',
                en: 'Thetford Cape',

            },
        },
        {
            itemId: 'FW_CAERLEON',
            itemName: {
                ru: 'Плащ города Caerleon',
                en: 'Caerleon Cape',

            },
        },
        {
            itemId: 'HERETIC',
            itemName: {
                ru: 'Плащ Еретиков',
                en: 'Heretic Cape',

            },
        },
        {
            itemId: 'UNDEAD',
            itemName: {
                ru: 'Плащ Нежити',
                en: 'Undead Cape',

            },
        },
        {
            itemId: 'KEEPER',
            itemName: {
                ru: 'Плащ Хранителей',
                en: 'Keeper Cape',

            },
        },
        {
            itemId: 'MORGANA',
            itemName: {
                ru: 'Плащ Морганы',
                en: 'Morgana Cape',

            },
        },
        {
            itemId: 'DEMON',
            itemName: {
                ru: 'Плащ Демонов',
                en: 'Demon Cape',

            },
        },
    ],
    "ARMOR": [
        {
            itemId: 'CLOTH_SET1',
            itemName: {
                ru: 'Мантия ученого',
                en: 'Scholar Robe',

            },
        },
        {
            itemId: 'CLOTH_SET2',
            itemName: {
                ru: 'Мантия клирика',
                en: 'Cleric Robe',

            },
        },
        {
            itemId: 'CLOTH_SET3',
            itemName: {
                ru: 'Мантия чародея',
                en: 'Mage Robe',

            },
        },
        {
            itemId: 'CLOTH_KEEPER',
            itemName: {
                ru: 'Мантия друида',
                en: 'Druid Robe',

            },
        },
        {
            itemId: 'CLOTH_MORGANA',
            itemName: {
                ru: 'Мантия культиста',
                en: 'Cultist Robe',

            },
        },
        {
            itemId: 'CLOTH_HELL',
            itemName: {
                ru: 'Дьявольская мантия',
                en: 'Fiend Robe',

            },
        },
        {
            itemId: 'CLOTH_FEY',
            itemName: {
                ru: 'Мантия из сказочной чешуи',
                en: 'Feyscale Robe',

            },
        },
        {
            itemId: 'CLOTH_AVALON',
            itemName: {
                ru: 'Мантия чистоты',
                en: 'Robe of Purity',

            },
        },
        {
            itemId: 'CLOTH_ROYAL',
            itemName: {
                ru: 'Королевская мантия',
                en: 'Royal Robe',

            },
        },
        {
            itemId: 'LEATHER_SET1',
            itemName: {
                ru: 'Куртка наемника',
                en: 'Mercenary Jacket',

            },
        },
        {
            itemId: 'LEATHER_SET2',
            itemName: {
                ru: 'Куртка охотника',
                en: 'Hunter Jacket',

            },
        },
        {
            itemId: 'LEATHER_SET3',
            itemName: {
                ru: 'Куртка убийцы',
                en: 'Assassin Jacket',

            },
        },
        {
            itemId: 'LEATHER_MORGANA',
            itemName: {
                ru: 'Куртка лазутчика',
                en: 'Stalker Jacket',

            },
        },
        {
            itemId: 'LEATHER_HELL',
            itemName: {
                ru: 'Адская куртка',
                en: 'Hellion Jacket',

            },
        },
        {
            itemId: 'LEATHER_UNDEAD',
            itemName: {
                ru: 'Куртка призрака',
                en: 'Specter Jacket',

            },
        },
        {
            itemId: 'LEATHER_FEY',
            itemName: {
                ru: 'Куртка туманного охотника',
                en: 'Mistwalker Jacket',

            },
        },
        {
            itemId: 'LEATHER_AVALON',
            itemName: {
                ru: 'Куртка упорства',
                en: 'Jacket of Tenacity',

            },
        },
        {
            itemId: 'LEATHER_ROYAL',
            itemName: {
                ru: 'Королевская куртка',
                en: 'Royal Jacket',

            },
        },
        {
            itemId: 'PLATE_SET1',
            itemName: {
                ru: 'Броня солдата',
                en: 'Soldier Armor',

            },
        },
        {
            itemId: 'PLATE_SET2',
            itemName: {
                ru: 'Броня рыцаря',
                en: 'Knight Armor',

            },
        },
        {
            itemId: 'PLATE_SET3',
            itemName: {
                ru: 'Броня хранителя',
                en: 'Guardian Armor',

            },
        },
        {
            itemId: 'PLATE_UNDEAD',
            itemName: {
                ru: 'Могильная броня',
                en: 'Graveguard Armor',

            },
        },
        {
            itemId: 'PLATE_HELL',
            itemName: {
                ru: 'Демоническая броня',
                en: 'Demon Armor',

            },
        },
        {
            itemId: 'PLATE_KEEPER',
            itemName: {
                ru: 'Броня вершителя',
                en: 'Judicator Armor',

            },
        },
        {
            itemId: 'PLATE_FEY',
            itemName: {
                ru: 'Броня сумеречного ткача',
                en: 'Duskweaver Armor',

            },
        },
        {
            itemId: 'PLATE_AVALON',
            itemName: {
                ru: 'Броня доблести',
                en: 'Armor of Valor',

            },
        },
        {
            itemId: 'PLATE_ROYAL',
            itemName: {
                ru: 'Королевская броня',
                en: 'Royal Armor',

            },
        },
        {
            itemId: 'GATHERER_FIBER',
            itemName: {
                ru: 'Одежда жнеца',
                en: 'Harvester Garb',

            },
        },
        {
            itemId: 'GATHERER_FISH',
            itemName: {
                ru: 'Одежда рыбака',
                en: 'Fisherman Garb',

            },
        },
        {
            itemId: 'GATHERER_HIDE',
            itemName: {
                ru: 'Одежда свежевателя',
                en: 'Skinner Garb',

            },
        },
        {
            itemId: 'GATHERER_ORE',
            itemName: {
                ru: 'Одежда шахтера',
                en: 'Miner Garb',

            },
        },
        {
            itemId: 'GATHERER_ROCK',
            itemName: {
                ru: 'Одежда каменолома',
                en: 'Quarrier Garb',

            },
        },
        {
            itemId: 'GATHERER_WOOD',
            itemName: {
                ru: 'Одежда дровосека',
                en: 'Lumberjack Garb',

            },
        },
    ],
    "HEAD": [
        {
            itemId: 'CLOTH_SET1',
            itemName: {
                ru: 'Колпак ученого',
                en: 'Scholar Cowl',

            },
        },
        {
            itemId: 'CLOTH_SET2',
            itemName: {
                ru: 'Колпак клирика',
                en: 'Cleric Cowl',

            },
        },
        {
            itemId: 'CLOTH_SET3',
            itemName: {
                ru: 'Колпак чародея',
                en: 'Mage Cowl',

            },
        },
        {
            itemId: 'CLOTH_KEEPER',
            itemName: {
                ru: 'Колпак друида',
                en: 'Druid Cowl',

            },
        },
        {
            itemId: 'CLOTH_HELL',
            itemName: {
                ru: 'Дьявольский колпак',
                en: 'Fiend Cowl',

            },
        },
        {
            itemId: 'CLOTH_MORGANA',
            itemName: {
                ru: 'Колпак культиста',
                en: 'Cultist Cowl',

            },
        },
        {
            itemId: 'CLOTH_FEY',
            itemName: {
                ru: 'Шляпа из сказочной чешуи',
                en: 'Feyscale Hat',

            },
        },
        {
            itemId: 'CLOTH_AVALON',
            itemName: {
                ru: 'Колпак чистоты',
                en: 'Cowl of Purity',

            },
        },
        {
            itemId: 'CLOTH_ROYAL',
            itemName: {
                ru: 'Королевский колпак',
                en: 'Royal Cowl',

            },
        },
        {
            itemId: 'LEATHER_SET1',
            itemName: {
                ru: 'Капюшон наемника',
                en: 'Mercenary Hood',

            },
        },
        {
            itemId: 'LEATHER_SET2',
            itemName: {
                ru: 'Капюшон охотника',
                en: 'Hunter Hood',

            },
        },
        {
            itemId: 'LEATHER_SET3',
            itemName: {
                ru: 'Капюшон убийцы',
                en: 'Assassin Hood',

            },
        },
        {
            itemId: 'LEATHER_MORGANA',
            itemName: {
                ru: 'Капюшон лазутчика',
                en: 'Stalker Hood',

            },
        },
        {
            itemId: 'LEATHER_HELL',
            itemName: {
                ru: 'Адский капюшон',
                en: 'Hellion Hood',

            },
        },
        {
            itemId: 'LEATHER_UNDEAD',
            itemName: {
                ru: 'Капюшон призрака',
                en: 'Specter Hood',

            },
        },
        {
            itemId: 'LEATHER_FEY',
            itemName: {
                ru: 'Капюшон туманного охотника',
                en: 'Mistwalker Hood',

            },
        },
        {
            itemId: 'LEATHER_AVALON',
            itemName: {
                ru: 'Капюшон упорства',
                en: 'Hood of Tenacity',

            },
        },
        {
            itemId: 'LEATHER_ROYAL',
            itemName: {
                ru: 'Королевский капюшон',
                en: 'Royal Hood',

            },
        },
        {
            itemId: 'PLATE_SET1',
            itemName: {
                ru: 'Шлем солдата',
                en: 'Soldier Helmet',

            },
        },
        {
            itemId: 'PLATE_SET2',
            itemName: {
                ru: 'Шлем рыцаря',
                en: 'Knight Helmet',

            },
        },
        {
            itemId: 'PLATE_SET3',
            itemName: {
                ru: 'Шлем хранителя',
                en: 'Guardian Helmet',

            },
        },
        {
            itemId: 'PLATE_UNDEAD',
            itemName: {
                ru: 'Могильный шлем',
                en: 'Graveguard Helmet',

            },
        },
        {
            itemId: 'PLATE_HELL',
            itemName: {
                ru: 'Демонический шлем',
                en: 'Demon Helmet',

            },
        },
        {
            itemId: 'PLATE_KEEPER',
            itemName: {
                ru: 'Шлем вершителя',
                en: 'Judicator Helmet',

            },
        },
        {
            itemId: 'PLATE_FEY',
            itemName: {
                ru: 'Шлем сумеречного ткача',
                en: 'Duskweaver Helmet',

            },
        },
        {
            itemId: 'PLATE_AVALON',
            itemName: {
                ru: 'Шлем доблести',
                en: 'Helmet of Valor',

            },
        },
        {
            itemId: 'PLATE_ROYAL',
            itemName: {
                ru: 'Королевский шлем',
                en: 'Royal Helmet',

            },
        },
        {
            itemId: 'GATHERER_FIBER',
            itemName: {
                ru: 'Шапка жнеца',
                en: 'Harvester Cap',

            },
        },
        {
            itemId: 'GATHERER_FISH',
            itemName: {
                ru: 'Шапка рыбака',
                en: 'Fisherman Cap',

            },
        },
        {
            itemId: 'GATHERER_HIDE',
            itemName: {
                ru: 'Шапка свежевателя',
                en: 'Skinner Cap',

            },
        },
        {
            itemId: 'GATHERER_ORE',
            itemName: {
                ru: 'Шапка шахтера',
                en: 'Miner Cap',

            },
        },
        {
            itemId: 'GATHERER_ROCK',
            itemName: {
                ru: 'Шапка каменолома',
                en: 'Quarrier Cap',

            },
        },
        {
            itemId: 'GATHERER_WOOD',
            itemName: {
                ru: 'Шапка дровосека',
                en: 'Lumberjack Cap',

            },
        },
    ],
    "SHOES": [
        {
            itemId: 'CLOTH_SET1',
            itemName: {
                ru: 'Сандалии ученого',
                en: 'Scholar Sandals',

            },
        },
        {
            itemId: 'CLOTH_SET2',
            itemName: {
                ru: 'Сандалии клирика',
                en: 'Cleric Sandals',

            },
        },
        {
            itemId: 'CLOTH_SET3',
            itemName: {
                ru: 'Сандалии чародея',
                en: 'Mage Sandals',

            },
        },
        {
            itemId: 'CLOTH_KEEPER',
            itemName: {
                ru: 'Сандалии друида',
                en: 'Druid Sandals',

            },
        },
        {
            itemId: 'CLOTH_HELL',
            itemName: {
                ru: 'Дьявольские сандалии',
                en: 'Fiend Sandals',

            },
        },
        {
            itemId: 'CLOTH_MORGANA',
            itemName: {
                ru: 'Сандалии культиста',
                en: 'Cultist Sandals',

            },
        },
        {
            itemId: 'CLOTH_FEY',
            itemName: {
                ru: 'Сандалии из сказочной чешуи',
                en: 'Feyscale Sandals',

            },
        },
        {
            itemId: 'CLOTH_AVALON',
            itemName: {
                ru: 'Сандалии чистоты',
                en: 'Sandals of Purity',

            },
        },
        {
            itemId: 'CLOTH_ROYAL',
            itemName: {
                ru: 'Королевские сандалии',
                en: 'Royal Sandals',

            },
        },
        {
            itemId: 'LEATHER_SET1',
            itemName: {
                ru: 'Ботинки наемника',
                en: 'Mercenary Shoes',

            },
        },
        {
            itemId: 'LEATHER_SET2',
            itemName: {
                ru: 'Ботинки охотника',
                en: 'Hunter Shoes',

            },
        },
        {
            itemId: 'LEATHER_SET3',
            itemName: {
                ru: 'Ботинки убийцы',
                en: 'Assassin Shoes',

            },
        },
        {
            itemId: 'LEATHER_MORGANA',
            itemName: {
                ru: 'Ботинки лазутчика',
                en: 'Stalker Shoes',

            },
        },
        {
            itemId: 'LEATHER_HELL',
            itemName: {
                ru: 'Адские ботинки',
                en: 'Hellion Shoes',

            },
        },
        {
            itemId: 'LEATHER_UNDEAD',
            itemName: {
                ru: 'Ботинки призрака',
                en: 'Specter Shoes',

            },
        },
        {
            itemId: 'LEATHER_FEY',
            itemName: {
                ru: 'Ботинки туманного охотника',
                en: 'Mistwalker Shoes',

            },
        },
        {
            itemId: 'LEATHER_AVALON',
            itemName: {
                ru: 'Ботинки упорства',
                en: 'Shoes of Tenacity',

            },
        },
        {
            itemId: 'LEATHER_ROYAL',
            itemName: {
                ru: 'Королевские ботинки',
                en: 'Royal Shoes',

            },
        },
        {
            itemId: 'PLATE_SET1',
            itemName: {
                ru: 'Сапоги солдата',
                en: 'Soldier Boots',

            },
        },
        {
            itemId: 'PLATE_SET2',
            itemName: {
                ru: 'Сапоги рыцаря',
                en: 'Knight Boots',

            },
        },
        {
            itemId: 'PLATE_SET3',
            itemName: {
                ru: 'Сапоги хранителя',
                en: 'Guardian Boots',

            },
        },
        {
            itemId: 'PLATE_UNDEAD',
            itemName: {
                ru: 'Могильные сапоги',
                en: 'Graveguard Boots',

            },
        },
        {
            itemId: 'PLATE_HELL',
            itemName: {
                ru: 'Демонические сапоги',
                en: 'Demon Boots',

            },
        },
        {
            itemId: 'PLATE_KEEPER',
            itemName: {
                ru: 'Сапоги вершителя',
                en: 'Judicator Boots',

            },
        },
        {
            itemId: 'PLATE_FEY',
            itemName: {
                ru: 'Сапоги сумеречного ткача',
                en: 'Duskweaver Boots',

            },
        },
        {
            itemId: 'PLATE_AVALON',
            itemName: {
                ru: 'Сапоги доблести',
                en: 'Boots of Valor',

            },
        },
        {
            itemId: 'PLATE_ROYAL',
            itemName: {
                ru: 'Королевские сапоги',
                en: 'Royal Boots',

            },
        },
        {
            itemId: 'GATHERER_FIBER',
            itemName: {
                ru: 'Рабочие сапоги жнеца',
                en: 'Harvester Workboots',

            },
        },
        {
            itemId: 'GATHERER_FISH',
            itemName: {
                ru: 'Рабочие сапоги рыбака',
                en: 'Fisherman Workboots',

            },
        },
        {
            itemId: 'GATHERER_HIDE',
            itemName: {
                ru: 'Рабочие сапоги свежевателя',
                en: 'Skinner Workboots',

            },
        },
        {
            itemId: 'GATHERER_ORE',
            itemName: {
                ru: 'Рабочие сапоги шахтера',
                en: 'Miner Workboots',

            },
        },
        {
            itemId: 'GATHERER_ROCK',
            itemName: {
                ru: 'Рабочие сапоги каменолома',
                en: 'Quarrier Workboots',

            },
        },
        {
            itemId: 'T4_SHOES_GATHERER_WOOD',
            itemName: {
                ru: 'Рабочие сапоги дровосека',
                en: 'Lumberjack Workboots',

            },
        },
    ],
    "OFF": [
        {
            itemId: 'SHIELD',
            itemName: {
                ru: 'Щит',
                en: 'Shield',

            },
        },
        {
            itemId: 'TOWERSHIELD_UNDEAD',
            itemName: {
                ru: 'Саркофаг',
                en: 'Sarcophagus',

            },
        },
        {
            itemId: 'SHIELD_HELL',
            itemName: {
                ru: 'Щит негодяя',
                en: 'Caitiff Shield',

            },
        },
        {
            itemId: 'SPIKEDSHIELD_MORGANA',
            itemName: {
                ru: 'Лицелом',
                en: 'Facebreaker',

            },
        },
        {
            itemId: 'SHIELD_AVALON',
            itemName: {
                ru: 'Эгида звезд',
                en: 'Astral Aegis',

            },
        },
        {
            itemId: 'BOOK',
            itemName: {
                ru: 'Книга заклинаний',
                en: 'Tome of Spells',

            },
        },
        {
            itemId: 'ORB_MORGANA',
            itemName: {
                ru: 'Око тайны',
                en: 'Eye of Secrets',

            },
        },
        {
            itemId: 'DEMONSKULL_HELL',
            itemName: {
                ru: 'Череп-трофей',
                en: 'Muisak',

            },
        },
        {
            itemId: 'TOTEM_KEEPER',
            itemName: {
                ru: 'Корень природы',
                en: 'Taproot',

            },
        },
        {
            itemId: 'CENSER_AVALON',
            itemName: {
                ru: 'Небесное кадило',
                en: 'Celestial Censer',

            },
        },
        {
            itemId: 'TORCH',
            itemName: {
                ru: 'Факел',
                en: 'Torch',

            },
        },
        {
            itemId: 'HORN_KEEPER',
            itemName: {
                ru: 'Туманный зов',
                en: 'Mistcaller',

            },
        },
        {
            itemId: 'TALISMAN_AVALON',
            itemName: {
                ru: 'Священный скипетр',
                en: 'Sacred Scepter',

            },
        },
        {
            itemId: 'LAMP_UNDEAD',
            itemName: {
                ru: 'Могильная свеча',
                en: 'Cryptcandle',

            },
        },
        {
            itemId: 'JESTERCANE_HELL',
            itemName: {
                ru: 'Зловещая трость',
                en: 'Leering Cane',

            },
        },
    ],
    "LEATHER": [
        {
            itemId: 'LEATHER',
            itemName: {
                ru: 'Кожа',
                en: 'Leather',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Кожа',
                en: 'Leather',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Кожа',
                en: 'Leather',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Кожа',
                en: 'Leather',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Кожа',
                en: 'Leather',

            },
        },
    ],
    "CLOTH": [
        {
            itemId: 'CLOTH',
            itemName: {
                ru: 'Ткань',
                en: 'Cloth',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Ткань',
                en: 'Cloth',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Ткань',
                en: 'Cloth',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Ткань',
                en: 'Cloth',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Ткань',
                en: 'Cloth',

            },
        },
    ],
    "METALBAR": [
        {
            itemId: 'METALBAR',
            itemName: {
                ru: 'Слиток',
                en: 'Bar',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Слиток',
                en: 'Bar',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Слиток',
                en: 'Bar',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Слиток',
                en: 'Bar',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Слиток',
                en: 'Bar',

            },
        },
    ],
    "PLANKS": [
        {
            itemId: 'PLANKS',
            itemName: {
                ru: 'Брусья',
                en: 'Planks',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Брусья',
                en: 'Planks',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Брусья',
                en: 'Planks',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Брусья',
                en: 'Planks',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Брусья',
                en: 'Planks',

            },
        },
    ],
    "STONEBLOCK": [
        {
            itemId: 'STONEBLOCK',
            itemName: {
                ru: 'Блок',
                en: 'Block',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Блок',
                en: 'Block',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Блок',
                en: 'Block',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Блок',
                en: 'Block',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Блок',
                en: 'Block',

            },
        },
    ],
    "FIBER": [
        {
            itemId: 'FIBER',
            itemName: {
                ru: 'Волокно',
                en: 'Fiber',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Волокно',
                en: 'Fiber',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Волокно',
                en: 'Fiber',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Волокно',
                en: 'Fiber',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Волокно',
                en: 'Fiber',

            },
        },
    ],
    "HIDE": [
        {
            itemId: 'HIDE',
            itemName: {
                ru: 'Шкура',
                en: 'Hide',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Шкура',
                en: 'Hide',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Шкура',
                en: 'Hide',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Шкура',
                en: 'Hide',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Шкура',
                en: 'Hide',

            },
        },
    ],
    "ORE": [
        {
            itemId: 'ORE',
            itemName: {
                ru: 'Руда',
                en: 'Ore',
            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Руда',
                en: 'Ore',
            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Руда',
                en: 'Ore',
            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Руда',
                en: 'Ore',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Руда',
                en: 'Ore',
            },
        },
    ],
    "ROCK": [
        {
            itemId: 'ROCK',
            itemName: {
                ru: 'Камень',
                en: 'Stone',
            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Камень',
                en: 'Stone',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Камень',
                en: 'Stone',

            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Камень',
                en: 'Stone',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Камень',
                en: 'Stone',
            },
        },
    ],
    "WOOD": [
        {
            itemId: 'WOOD',
            itemName: {
                ru: 'Бревна',
                en: 'Logs',

            },
        },
        {
            itemId: 'LEVEL1',
            itemName: {
                ru: 'Бревна',
                en: 'Logs',

            },
        },
        {
            itemId: 'LEVEL2',
            itemName: {
                ru: 'Бревна',
                en: 'Logs',
            },
        },
        {
            itemId: 'LEVEL3',
            itemName: {
                ru: 'Бревна',
                en: 'Logs',

            },
        },
        {
            itemId: 'LEVEL4',
            itemName: {
                ru: 'Бревна',
                en: 'Logs',

            },
        },
    ],
    "BACKPACK": [
        {
            itemId: 'GATHERER_FIBER',
            itemName: {
                ru: 'Рюкзак жнеца',
                en: 'Harvester Backpack',

            },
        },
        {
            itemId: 'GATHERER_FISH',
            itemName: {
                ru: 'Рюкзак рыбака',
                en: 'Fisherman Backpack',

            },
        },
        {
            itemId: 'GATHERER_HIDE',
            itemName: {
                ru: 'Рюкзак свежевателя',
                en: 'Skinner Backpack',

            },
        },
        {
            itemId: 'GATHERER_ORE',
            itemName: {
                ru: 'Рюкзак шахтера',
                en: 'Miner Backpack',

            },
        },
        {
            itemId: 'GATHERER_ROCK',
            itemName: {
                ru: 'Рюкзак каменолома',
                en: 'Quarrier Backpack',

            },
        },
        {
            itemId: 'GATHERER_WOOD',
            itemName: {
                ru: 'Рюкзак дровосека',
                en: 'Lumberjack Backpack',
            },
        },
    ],
    "SKILLBOOK": [
        {
            itemId: 'GATHER_FIBER',
            itemName: {
                ru: 'Фолиант жнеца',

                en: 'Fiber Harvester Tome',

            },
        },
        {
            itemId: 'GATHER_HIDE',
            itemName: {
                ru: 'Фолиант свежевателя',

                en: 'Animal Skinner Tome',

            },
        },
        {
            itemId: 'GATHER_ORE',
            itemName: {
                ru: 'Фолиант шахтера',

                en: 'Ore Miner Tome',

            },
        },
        {
            itemId: 'GATHER_ROCK',
            itemName: {
                ru: 'Фолиант каменолома',

                en: 'Quarrier Tome',

            },
        },
        {
            itemId: 'GATHER_WOOD',
            itemName: {
                ru: 'Фолиант дровосека',

                en: 'Lumberjack Tome',

            },
        },
        {
            itemId: 'STANDARD',
            itemName: {
                ru: 'Книга интуиции',

                en: 'Tome of Insight',

            },
        },
    ],
    "FACTION": [
        {
            itemId: 'FOREST_TOKEN_1',
            itemName: {
                ru: 'Древесное сердце',

                en: 'Treeheart',

            },
        },
        {
            itemId: 'HIGHLAND_TOKEN_1',
            itemName: {
                ru: 'Каменное сердце',

                en: 'Rockheart',

            },
        },
        {
            itemId: 'STEPPE_TOKEN_1',
            itemName: {
                ru: 'Звериное сердце',

                en: 'Beastheart',

            },
        },
        {
            itemId: 'MOUNTAIN_TOKEN_1',
            itemName: {
                ru: 'Сердце горы',

                en: 'Mountainheart',

            },
        },
        {
            itemId: 'SWAMP_TOKEN_1',
            itemName: {
                ru: 'Сердце лозы',

                en: 'Vineheart',

            },
        },
        {
            itemId: 'CAERLEON_TOKEN_1',
            itemName: {
                ru: 'Теневое сердце',

                en: 'Shadowheart',

            },
        },
    ],
    "RUNE": [
        {
            itemId: 'RUNE',
            itemName: {
                ru: 'Руна',

                en: 'Rune',

            },
        },
    ],
    "SOUL": [
        {
            itemId: 'SOUL',
            itemName: {
                ru: 'Душа',

                en: 'Soul',

            },
        },
    ],
    "RELIC": [
        {
            itemId: 'RELIC',
            itemName: {
                ru: 'Реликт',

                en: 'Relic',

            },
        },
    ],
    "SHARD": [
        {
            itemId: 'AVALONIAN',
            itemName: {
                ru: 'Осколок Авалона',

                en: 'Avalonian Shard',

            },
        },
    ],
    "JOURNAL": [
        {
            itemId: 'WOOD',
            itemName: {
                ru: 'Журнал дровосека(неполный)',

                en: 'Lumberjack\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'STONE',
            itemName: {
                ru: 'Журнал каменолома(неполный)',

                en: 'Stonecutter\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'ORE',
            itemName: {
                ru: 'Журнал шахтера(неполный)',

                en: 'Prospector\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'FIBER',
            itemName: {
                ru: 'Журнал жнеца(неполный)',

                en: 'Cropper\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'HIDE',
            itemName: {
                ru: 'Журнал свежевателя(неполный)',

                en: 'Gamekeeper\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'WARRIOR_EMPTY',
            itemName: {
                ru: 'Журнал кузнеца(пустой)',

                en: 'Blacksmith\'s Journal(empty)',

            },
        },
        {
            itemId: 'HUNTER_EMPTY',
            itemName: {
                ru: 'Журнал охотника(пустой)',

                en: 'Fletcher\'s Journal(empty)',

            },
        },
        {
            itemId: 'MAGE_EMPTY',
            itemName: {
                ru: 'Журнал мага(пустой)',

                en: 'Imbuer\'s Journal(empty)',

            },
        },
        {
            itemId: 'TOOLMAKER_EMPTY',
            itemName: {
                ru: 'Журнал слесаря(пустой)',

                en: 'Tinker\'s Journal(empty)',

            },
        },
        {
            itemId: 'MERCENARY',
            itemName: {
                ru: 'Журнал наемника(неполный)',

                en: 'Mercenary\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_GENERAL',
            itemName: {
                ru: 'Общий трофейный журнал(неполный)',

                en: 'Generalist Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_MERCENARY',
            itemName: {
                ru: 'Трофейный журнал наемника(неполный)',

                en: 'Mercenary\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_HIDE',
            itemName: {
                ru: 'Трофейный журнал свежевателя(неполный)',

                en: 'Gamekeeper\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_WOOD',
            itemName: {
                ru: 'Трофейный журнал дровосека(неполный)',

                en: 'Lumberjack\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_STONE',
            itemName: {
                ru: 'Трофейный журнал каменолома(неполный)',

                en: 'Stonecutter\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_ORE',
            itemName: {
                ru: 'Трофейный журнал шахтера(неполный)',

                en: 'Prospector\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_FIBER',
            itemName: {
                ru: 'Трофейный журнал жнеца(неполный)',

                en: 'Cropper\'s Trophy Journal(Partially Full)',

            },
        },
        {
            itemId: 'FISHING',
            itemName: {
                ru: 'Журнал рыбака(неполный)',

                en: 'Fisherman\'s Journal(Partially Full)',

            },
        },
        {
            itemId: 'TROPHY_FISHING',
            itemName: {
                ru: 'Трофейный журнал рыбака(неполный)',

                en: 'Fisherman\'s Trophy Journal(Partially Full)',

            },
        },
    ],
};