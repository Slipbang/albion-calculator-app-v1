...
export class CraftedConsumablesInfoClass {
    constructor(
        ...
        public consumableNames: TConsumableNames | undefined,
    ...
    consumableResourcePrices = () => {
        ...
        consumableResourcesKeys!.forEach(key => {
            const finalKey = key !== 'FISHSAUCE' ? key : `T1_FISHSAUCE_LEVEL${enchantment}`
    ...
    resourcePricesCalculation = () => {
        const {consumableResourcesKeys, infoTableData, percent, consumableNames, selectedLanguage, resourcePrices, enchantment, consumableItemPrice, totalFoodTax} = this;
        ...
        consumableResourcesKeys?.map(key => {
            if (!enchantment && key === 'FISHSAUCE') return;
            ...
            resourcesPricesTitle.push(`${consumableNames![key !== 'FISHSAUCE' ? key : `T1_FISHSAUCE_LEVEL${enchantment}`][selectedLanguage]}: ${resourcePrices[key].toLocaleString('en')} * ${totalQuantity} = ${totalItemCost.toLocaleString('en')}<hr><br>`);
            totalCost += totalItemCost;
        })

        const profitPerItem = (+consumableItemPrice * craftedFood.quantity - totalCost);
        const profitPerItemTitle = `${consumableItemPrice.toLocaleString('en')} * ${craftedFood.quantity} - ${totalCost.toLocaleString('en')} = ${profitPerItem.toLocaleString('en')}`
    ...
    itemPriceTitle = `${this.consumableNames![this.infoTableData.craftedFood.itemId][this.selectedLanguage]}: ${this.consumableItemPrice.toLocaleString('en')}`;
    ...
    totalProfitTitle = `${this.infoTableStrings.total} ${this.totalProfit.toLocaleString('en')}`
